...
// Gate 3B's contract evidence is recorded here as deployment metadata, not
// in UI logic.  Production stays disabled in every definition.
const STAGING_CONTRACT_PASS_IDS = new Set([
...
export const IMAGE_MODELS = Object.freeze(RAW_IMAGE_MODELS.map((model) => Object.freeze({
  ...model,
  deployments: Object.freeze({ ...model.deployments, staging: STAGING_CONTRACT_PASS_IDS.has(model.id) ? "STAGING_ENABLED" : model.deployments.staging }),
})));
