...
export default function ProductAdForm({
  ...
  return (
    <div className="space-y-4 text-xs font-sans text-gray-200">
      ...
      <div className="space-y-1.5">
        <div className="flex items-center gap-1">
          <label className="block text-xs font-semibold text-gray-300">
            Upload your product <span className="text-red-400">*</span>
          </label>
          <FiHelpCircle size={12} className="text-gray-500" title="Upload a photo of your physical product" />
        </div>
        <div className="flex items-center gap-3">
          {productImage ? (
            <div className="relative w-20 h-20 rounded-xl border border-white/10 overflow-hidden group">
              <img src={productImage.preview || productImage.url} alt="Product" className="w-full h-full object-cover" />
              <button
                type="button"
            ...
            <label className="w-20 h-20 rounded-xl bg-[#1c1c20] border border-dashed border-white/20 hover:border-white/40 flex flex-col items-center justify-center cursor-pointer transition-colors">
              ...
              <input type="file" accept="image/*" onChange={onProductUpload} className="hidden" />
            </label>
          )}
        </div>
      </div>
      ...
      <div className="space-y-1.5">
        <div className="flex items-center gap-1">
          <label className="block text-xs font-semibold text-gray-300">Additional references</label>
          <FiHelpCircle size={12} className="text-gray-500" title="Optional reference imagery" />
        </div>
        <div className="flex flex-wrap gap-2">
          {uploadedImages?.filter(i => i.id !== productImage?.id).map((img) => (
